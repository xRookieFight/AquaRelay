...
final class BackendRakClient{
    ...
    private function sendRequest2() : void{
        ...
        $s->putByte(0x07); 
        $s->put(self::MAGIC);
        $s->putByte(4); $s->putByte(127); $s->putByte(0); $s->putByte(0); $s->putByte(1); $s->putShort(0);
        $s->putShort($this->mtu);
    ...
    private function handleDatagram(string $buf, callable $onPacket) : void{
        ...
        while(!$s->feof()){
            try {
                ...
                if($reliability >= 2) $s->getLTriad();
                if(in_array($reliability, [3,6,7], true)) { $s->getLTriad(); $s->getByte(); }

                if($split){
                    $count = $s->getInt(); $id = $s->getShort(); $index = $s->getInt();
    ...
    private function processPayload(string $payload, callable $onPacket) : void{
        ...
        if($pid === 0x10 && $this->state === self::STATE_CONNECTING_3){
        ...
        if ($pid === 0xFE) {
            if ($this->state === self::STATE_GAME_HANDSHAKE) {
    ...
    private function handleInternalPacket(string $buf) : void{
        ...
        switch(ord($buf[0])){
            case 0x06:
                $pk = new OpenConnectionReply1(); $pk->decode($s);
                $this->mtu = min($this->mtu, $pk->mtuSize);
                $this->sendRequest2();
                break;
            case 0x08:
                $pk = new OpenConnectionReply2(); $pk->decode($s);
                $this->mtu = $pk->mtuSize;
    ...
    private function sendAck(string $buf) : void{
        ...
        $s = new Stream();
        $s->putByte(0xC0); $s->putShort(1); $s->putByte(1); 
        $s->putLTriad($seq); $s->putLTriad($seq);
        $this->sendRaw($s->getBuffer());
    }

    private function sendRawPacket(Packet $packet) : void{
        $s = new Stream(); $packet->encode($s);
        $this->sendRaw($s->getBuffer());
    }
