...
class Utils
{
    ...
    public static function getOS(bool $recalculate = false): string
    {
        if (null === self::$os || $recalculate) {
            ...
            if (false !== stripos($uname, 'Win') || 'Msys' === $uname) {
            ...
            self::$os = match (true) {
                false !== stripos($uname, 'Darwin') => str_starts_with($machine, 'iP')
                    ? self::OS_IOS
                    : self::OS_MACOS,

                false !== stripos($uname, 'Linux') => @file_exists('/system/build.prop')
                    ? self::OS_ANDROID
                    : self::OS_LINUX,

                false !== stripos($uname, 'BSD'),
                'DragonFly' === $uname => self::OS_BSD,
